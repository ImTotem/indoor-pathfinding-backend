...
def _load_node_image_bgr(db_path: str, node_id: int) -> Optional[np.ndarray]:
    try:
        ...
        logger.warning(f"[SPMatchDebugger] Cannot load node {node_id} image: {exc}")
    return None
...
def visualize_matches_sp(
    ...
    loaded = SuperPointMapManager().get_or_load(map_id, db_path)
    ...
    q_kps = q_feats["keypoints"][0].cpu().numpy()

    gray_uint8 = (gray * 255).clip(0, 255).astype(np.uint8)
    q_global = GlobalDescExtractor(engine._device).extract(gray_uint8)
    ...
    query_bgr = cv2.cvtColor(gray_uint8, cv2.COLOR_GRAY2BGR)
